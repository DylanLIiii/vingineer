...
class CopilotConverter:
    ...
    def save(self, target_dir: Path):
        ...
        self._save_prompts(github_dir / "prompts")
        self._save_agents(github_dir / "agents")
        self._save_skills(github_dir / "skills")
        self._save_mcp(target_dir)

    def _save_prompts(self, prompts_dir: Path):
    ...
    def _save_skills(self, skills_dir: Path):
        ...
        for skill in self.config.skills:
            # For skills, we ideally want to copy the whole directory if source path is known
            # If loaded from memory without path, we recreate basic structure
